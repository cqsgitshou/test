...
import com.yogu.p2p.common.util.CollectionUtils;
import com.yogu.p2p.common.util.DateUtil;
import com.yogu.p2p.common.util.SpringContextHolder;
...
public class RedisTemplateUtil {
     ...
     * @param value
     */
    public static <T> Boolean hashSetByExpire(final String key, final String field, final T value,final long expire) {
        return redisTemplate.execute(new RedisCallback<Boolean>() {
            @Override
            public Boolean doInRedis(RedisConnection connection) throws DataAccessException {
                try {
                    byte[] _key = keySerializer.serialize(key);
                    connection.hSet(_key, keySerializer.serialize(field), fSTRedisSerializer.serialize(value));
                    connection.hSet(_key, keySerializer.serialize(field+"expire"), fSTRedisSerializer.serialize(System.currentTimeMillis()/1000 + expire));
                    return Boolean.TRUE;
                } catch (Exception e) {
                    LOGGER.error("redis redisTemplate execute hashSet!", e);
                    return Boolean.FALSE;
     ...
     */
    @SuppressWarnings("unchecked")
    public static <T> T hashGetByExpire(final String key, final String field) {
        return redisTemplate.execute(new RedisCallback<T>() {
            @Override
            public T doInRedis(RedisConnection connection) throws DataAccessException {
                try {
                    ...
                    byte[] _value = connection.hGet(_key, _field);
                    if (_value != null) {
                        long expire = (long) fSTRedisSerializer.deserialize(connection.hGet(_key, keySerializer.serialize(field+"expire")));
